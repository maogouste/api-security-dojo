...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            .route("/api/docs/mode", web::get().to(api::docs_mode))
            .route("/api/docs/stats", web::get().to(api::docs_stats))
            .route("/api/docs/categories", web::get().to(api::docs_categories))
            .route("/api/docs/vulnerabilities", web::get().to(api::docs_vulnerabilities))
            .route("/api/docs/vulnerabilities/{id}", web::get().to(api::docs_vulnerability))
            .route("/api/docs/compare", web::get().to(api::docs_compare_list))
            .route("/api/docs/compare/{id}", web::get().to(api::docs_compare))
            // GraphQL (G01-G05)
